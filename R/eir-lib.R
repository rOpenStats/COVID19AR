...
COVID19ARpreprocessor <- R6Class("COVID19ARpreprocessor",
  public = list(
    ...
    preprocessCases = function(force.preprocess = FALSE) {
      ...
      if (!file.exists(cases.preprocessed.filepath) | force.preprocess) {
        ...
        if (download) {
          ...
          download.file("https://sisa.msal.gov.ar/datos/descargas/covid-19/files/Covid19Casos.zip",
            destfile = cases.zip.path,
            method = "wget",
            mode = "wb"
          )
        }
        ...
        self$cases.agg.df <-
          ...
          group_by(
            year_fis, sepi_fis, residencia_provincia_nombre,
            residencia_departamento_nombre, grupo_etario
          ) %>%
          summarize(
            fallecidos = sum(ifelse(fallecido == "SI" & clasificacion_resumen == "Confirmado", 1, 0)),
        ...
        self$cases.agg.df <- read_delim(
          file = cases.preprocessed.filepath, delim = ";",
          col_types = cols(
            year_fis = col_number(),
            sepi_fis = col_number(),
            residencia_provincia_nombre = col_character(),
    ...
    preprocessVaccines = function(force.preprocess = FALSE) {
      ...
      if (download) {
        # https://sisa.msal.gov.ar/datos/descargas/covid-19/files/Covid19Casos.zip
        download.file("https://sisa.msal.gov.ar/datos/descargas/covid-19/files/datos_nomivac_covid19.zip",
          destfile = vaccines.zip.path,
          mode = "wb"
        )
      }
      ...
      if (!file.exists(vaccines.preprocessed.filepath) | force.preprocess) {
        ...
        if (self$use.sqlfd) {
          sepi.fis.df <- self$cases.agg.df %>%
            ...
            summarize(n = n())
          self$vaccines.agg.df <- generateAggregatedVaccinesWithSqldf(vaccines.filepath, sepi.fis.df,
            total.chunks = 2,
            logger = logger
          )
        ...
        self$vaccines.agg.df <- read_delim(
          file = vaccines.preprocessed.filepath, delim = ";",
          col_types = cols(
            year_fis = col_number(),
            week_fis = col_number(),
            jurisdiccion_residencia = col_character(),
    ...
    fixVaccinesAgg = function() {
      ...
      # Match departamentos
      vaccines.departamentos <- self$vaccines.agg.df %>%
        group_by(jurisdiccion_residencia, depto_residencia) %>%
        summarize(n = n())
      covidstats.departamentos <- self$provincias.departamentos.edad.df %>%
      ...
      anti.join <- vaccines.departamentos %>% anti_join(covidstats.departamentos, by = c(
        "jurisdiccion_checked",
      ...
      vaccines.agg.covidstats.df %>%
        filter(jurisdiccion_checked == "CABA" & grupo_etario == "70-79") %>%
        arrange(desc(cum.applications))
      ...
      vaccines.agg.region.df <- vaccines.agg.covidstats.df %>%
        ...
        summarize(
          cum.applications = max(cum.applications),
      ...
      region.trim <- 0.4
      vaccines.agg.region.agg.df <- vaccines.agg.region.df %>%
      ...
      vaccines.agg.provincia.df <- vaccines.agg.covidstats.df %>%
        ...
        group_by(jurisdiccion_checked, grupo_etario, orden_dosis) %>%
        summarize(
          cobertura.1.dosis = sum(cum.applications) / sum(personas.covidstats),
          n = n()
        )
      ...
      vaccines.agg.covidstats.df %>%
        filter(personas.covidstats < personas.1.dosis) %>%
        select(
          prov = jurisdiccion_checked, dep = departamento_checked, ge = grupo_etario,
    ...
    LoadDataFromCovidStats = function(force.download = FALSE) {
      ...
      if (file.exists(vacunaciones.filepath)) {
        ...
        departamentos.catalog.df <- data.frame(
          idx = numeric(), codigo = character(),
        ...
        for (i in seq_len(length(vacunaciones.json))) {
          ...
          if (is.null(vacunaciones.departamento.json$poblacion)) {
            ...
            logger$warn("No population data for",
              i = i,
      ...
      for (i in seq_len(nrow(provincias.df))) {
        ...
        for (j in seq_len(nrow(provincia.departamentos.df))) {
          ...
          provincia.departamento.edad.df %<>% mutate(
            departamento = provincia.departamento.df$departamento,
    ...
    makeEcologicalInference = function(prediction.field = "cuidados_intensivos") {
      ...
      for (i in seq_len(nrow(year.week.df))) {
        ...
        vaccines.week.df.not.vaccunated <- vaccines.week.df %>%
          group_by(provincia_departamento, grupo_etario, personas) %>%
          summarize(applications.perc = 1 - sum(applications.perc))
